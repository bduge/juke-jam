...
const selectSong = (songObj, roomName) => {
    ...
    fetch(`${process.env.REACT_APP_API_URL}/add_song`, sendSongOptions)
        .then((data) => data.json())
...
const SearchBar = (props) => {
    ...
    useEffect(() => {
        ...
        socket.on('changeLike', (song, isLike) => {
            songDispatch(changeLike(song, isLike))
        })
    }, [])
    ...
    const searchSong = (event) => {
        ...
        fetch(`${process.env.REACT_APP_API_URL}/spotify/search`, searchOptions)
            .then((data) => data.json())
            .then((data) => {
                dispatch({
                    type: 'FINISH_SEARCH',
